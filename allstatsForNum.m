function R= allstatsForNum(A,varargin)
...
if ~isempty(varargin)
	...
	R= struct('min',zeros(s,1),'max',zeros(s,1),'mean',zeros(s,1),...
		...
		'skew',zeros(s,1),'gname',{});

    if isa(varargin{1,1},'double')      % Elements of group 'k'.  Using this in the for loop below
        rowString= 'find(factors == fval(k))';  
    else
